...
/* Includes ------------------------------------------------------------------*/
#include "../include/stm32f7xx_spi.h"

#include "../include/stm32f7xx_rcc.h"
...
/* Private define ------------------------------------------------------------*/
/* SPI registers Masks */
#define CR1_CLEAR_MASK       ((uint32_t)0x3040)
#define CR1_CLEAR_MASK2      ((uint32_t)0xFFFB)
#define CR2_LDMA_MASK        ((uint32_t)0x9FFF)

#define I2SCFGR_CLEAR_Mask   ((uint32_t)0xF040)
...
/*----------------------- SPIx I2SCFGR & I2SPR Configuration -----------------*/
  ...
  else
  {
    ...
    if(I2S_InitStruct->I2S_MCLKOutput == I2S_MCLKOutput_Enable)
    {
      ...
      tmp = (uint32_t)(((((sourceclock / 256) * 10) / I2S_InitStruct->I2S_AudioFreq)) + 5);
    }
    else
    {
      /* MCLK output is disabled */
      tmp = (uint32_t)(((((sourceclock / (32 * packetlength)) *10 ) / I2S_InitStruct->I2S_AudioFreq)) + 5);
    }
    ...
    i2sodd = (uint32_t)(tmp & (uint32_t)0x0001);
  ...
  SPIx->I2SPR = (uint32_t)(i2sdiv | (uint32_t)(i2sodd | (uint32_t)I2S_InitStruct->I2S_MCLKOutput));

  /* Configure the I2S with the SPI_InitStruct values */
  tmpreg |= (uint32_t)(SPI_I2SCFGR_I2SMOD | (uint32_t)(I2S_InitStruct->I2S_Mode | \
                  (uint32_t)(I2S_InitStruct->I2S_Standard | (uint32_t)(I2S_InitStruct->I2S_DataFormat | \
                  (uint32_t)I2S_InitStruct->I2S_CPOL))));
...
void SPI_TIModeCmd(SPI_TypeDef* SPIx, FunctionalState NewState)
{
  ...
  else
  {
    ...
    SPIx->CR2 &= (uint32_t)~((uint32_t)SPI_CR2_FRF);
  }
}
...
void I2S_Cmd(SPI_TypeDef* SPIx, FunctionalState NewState)
{
  ...
  else
  {
    ...
    SPIx->I2SCFGR &= (uint32_t)~((uint32_t)SPI_I2SCFGR_I2SE);
  }
}
...
void SPI_RxFIFOThresholdConfig(SPI_TypeDef* SPIx, uint32_t SPI_RxFIFOThreshold)
{
  ...
  /* Clear FRXTH bit */
  SPIx->CR2 &= (uint32_t)~((uint32_t)SPI_CR2_FRXTH);
...
void SPI_NSSPulseModeCmd(SPI_TypeDef* SPIx, FunctionalState NewState)
{
  ...
  else
  {
    ...
    SPIx->CR2 &= (uint32_t)~((uint32_t)SPI_CR2_NSSP);    
  }
}

/**
  * @brief  Enables or disables the Asynchronous Start mode.
  * @note     This function can be called only after the I2S_Init() function has 
  *           been called. 
  * @param  SPIx: where x can be 1, 2 or 3 to select the SPI peripheral.
  * @param  NewState: new state of the Asynchronous Start mode.
  *         This parameter can be: ENABLE or DISABLE.
  * @retval None
  */
void I2S_AsynchronousStartConfig(SPI_TypeDef* SPIx, FunctionalState NewState)
{
  ...
  {
    /* Enable the NSS pulse management mode */
    SPIx->CR2 |= SPI_I2SCFGR_ASTRTEN;
  }
  else
  {
    /* Disable the NSS pulse management mode */
    SPIx->CR2 &= (uint32_t)~((uint32_t)SPI_I2SCFGR_ASTRTEN);    
  }
}
...
/**
  ...
  * @param  SPIx: where x can be 1, 2, 3, 4, 5 or 6 in SPI mode to select the SPI peripheral.
...
void SPI_CRCLengthConfig(SPI_TypeDef* SPIx, uint32_t SPI_CRCLength)
{
  ...
  /* Clear CRCL bit */
  SPIx->CR1 &= (uint32_t)~((uint32_t)SPI_CR1_CRCL);
...
void SPI_CalculateCRC(SPI_TypeDef* SPIx, FunctionalState NewState)
{
  ...
  else
  {
    ...
    SPIx->CR1 &= (uint32_t)~((uint32_t)SPI_CR1_CRCEN);
  }
}
...
void SPI_I2S_DMACmd(SPI_TypeDef* SPIx, uint32_t SPI_I2S_DMAReq, FunctionalState NewState)
{
  ...
  else
  {
    ...
    SPIx->CR2 &= (uint32_t)~SPI_I2S_DMAReq;
  }
}
...
void SPI_I2S_ITConfig(SPI_TypeDef* SPIx, uint8_t SPI_I2S_IT, FunctionalState NewState)
{
  ...
  /* Set the IT mask */
  itmask = (uint32_t)1 << (uint32_t)itpos;
...
uint32_t SPI_GetTransmissionFIFOStatus(SPI_TypeDef* SPIx)
{
  ...
  return (uint32_t)((SPIx->SR & SPI_SR_FTLVL));
}
...
uint32_t SPI_GetReceptionFIFOStatus(SPI_TypeDef* SPIx)
{
  ...
  return (uint32_t)((SPIx->SR & SPI_SR_FRLVL));
}
...
ITStatus SPI_I2S_GetITStatus(SPI_TypeDef* SPIx, uint8_t SPI_I2S_IT)
{
  ...
  uint32_t itpos = 0, itmask = 0, enablestatus = 0;
  ...
  if (((SPIx->SR & itpos) != (uint32_t)RESET) && enablestatus)
  {
    /* SPI_I2S_IT is set */
    bitstatus = SET;
  }
