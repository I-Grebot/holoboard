/* Includes ------------------------------------------------------------------*/
#include "../include/stm32f7xx_tim.h"
...
void TIM_UIFRemap(TIM_TypeDef* TIMx, FunctionalState NewState)
{
  ...
  else
  {
    ...
    TIMx->CR1 &= (uint32_t)~TIM_CR1_UIFREMAP;
  }  
}
...
void TIM_OC6Init(TIM_TypeDef* TIMx, TIM_OCInitTypeDef* TIM_OCInitStruct)
{
  ...
  if((TIMx == TIM1) || (TIMx == TIM8))
  {
    ...
    /* Set the Output Idle state */
    tmpcr2 |= (uint32_t)(TIM_OCInitStruct->TIM_OCIdleState << 18);
  }
  /* Write to TIMx CR2 */
  TIMx->CR2 = tmpcr2;
...
/**
  ...
  * @retval None
  */
void TIM_SelectOCxM(TIM_TypeDef* TIMx, uint32_t TIM_Channel, uint32_t TIM_OCMode) /* to be updated*/
{
  uint32_t tmp = 0;
  uint32_t tmp1 = 0;
...
void TIM_SelectMasterSlaveMode(TIM_TypeDef* TIMx, uint32_t TIM_MasterSlaveMode)
{
  ...
  /* Reset the MSM Bit */
  TIMx->SMCR &= (uint32_t)~TIM_SMCR_MSM;
                       ...
                       uint32_t TIM_ICFilter)
{
  ...
  /* Select the Input and set the filter */
  tmpccmr2 &= ((uint32_t)~TIM_CCMR1_CC1S) & ((uint32_t)~TIM_CCMR2_IC3F);
                       ...
                       uint32_t TIM_ICFilter)
{
  ...
  /* Select the Input and set the filter */
  tmpccmr2 &= ((uint32_t)~TIM_CCMR1_CC2S) & ((uint32_t)~TIM_CCMR1_IC2F);
